...
class _DownloadedRecordTarget(luigi.Target):
    ...
    def exists(self):
        ...
        if timestamp == datetime.datetime.now().replace(minute=0, second=0, microsecond=0):
            Logger.info('Skipping download for {0}, wait for next hour.'.format(timestamp))
            return False
...
class DownloadRecordLogsTask(luigi.Task):
    ...
    def run(self):
        ...
        # Assuming that CloudFront generated multiple logs files for the latest hour,
        # downloaded files for current hour will never be complete, since we need
        # to wait for next hour to have all previous hour log files generated by CloudFront.
        if timestamp == datetime.datetime.now().replace(minute=0, second=0, microsecond=0):
            Logger.info('Skipping download for {0}, wait for next hour.'.format(timestamp))
            return
